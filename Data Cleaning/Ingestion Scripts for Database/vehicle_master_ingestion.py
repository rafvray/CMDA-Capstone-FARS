...
vehicle_master = pd.read_csv("/Users/rafaelviray/Downloads/master_vehicle.csv") # change to your path

vehicle_master.head(0).to_sql(
    name="vehicle_master",
    con=engine,
    if_exists="replace",  
    index=False
...
with engine.connect() as conn:
    result = conn.execute(text("SELECT COUNT(*) FROM vehicle_master;"))
    count = result.scalar() 
